...
def write_mp4(frames_array):
  length, width, height, fps = get_video_properties(video_path)
  out = cv2.VideoWriter('test.avi',cv2.VideoWriter_fourcc(*'DIVX'), fps, (width, height))
...
frames = generate_clip(video_path,start_frame=number, duration=5)
write_mp4(frames)
